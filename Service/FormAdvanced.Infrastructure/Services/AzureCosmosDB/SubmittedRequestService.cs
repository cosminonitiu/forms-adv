...
{
    public sealed class FormRequestService : IFormRequestService
    {
        private readonly Container _container;
        public FormRequestService(CosmosClient client, IOptions<AzureCosmosDBConfiguration> configurationOptions)
        {
            var config = configurationOptions.Value;
            _container = client
                .GetDatabase(config.DatabaseName)
                .GetContainer(config.FormContainerName);
        }
        ...
        public async Task<FormRequest?> GetByIdAsync(string id, string owner)
        {
            try
            {
                var response = await _container.ReadItemAsync<FormRequest>(
                    id,
        ...
        public async Task<List<FormRequest>> GetByOwnerAsync(string owner)
        {
            ...
            using var feed = _container.GetItemQueryIterator<FormRequest>(
                query,
            ...
            var results = new List<FormRequest>();
        ...
        {
            await _container.DeleteItemAsync<FormRequest>(
                id,
                new PartitionKey(owner)
            );
        }
